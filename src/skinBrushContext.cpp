...
MStatus SkinBrushContext::drawMeshWhileDrag(MHWRender::MUIDrawManager &drawManager)
{
    ...
    if (drawTransparency || drawPoints) {
        ...
        if ((commandIndex != ModifierCommands::Add) &&
            (commandIndex != ModifierCommands::AddPercent)) {
    ...
    for (const auto &pt : this->mirroredJoinedArray) {
        ...
        if (drawTriangles) {
            ...
            this->setColorWithMirror(
                ptIndex, weightBase, weightMirror, editVertsIndices, colors, colorsSolo
            );
    ...
    if (drawTriangles) {
        ...
        if (this->soloColorVal == 1) {
            drawManager.mesh(
                MHWRender::MUIDrawManager::kTriangles, points, &normals, &colorsSolo, &indices
            );
        }
        else {
            drawManager.mesh(
                MHWRender::MUIDrawManager::kTriangles, points, &normals, &colors, &indices
            );
        }
    }
    if (drawEdges) {
...
bool SkinBrushContext::getMirrorHit(bool getNormal, int &faceHit, MFloatPoint &hitPoint)
{
    ...
    if (getNormal) {
        meshFn.getClosestNormal(hitPoint, this->normalMirroredVector, MSpace::kWorld);
    }
    return true;
}
...
)
{
    ...
    if ((theCommandIndex == ModifierCommands::LockVertices) ||
        ...
        if (useMirror) {
            influenceColorIndex = this->mirrorInfluences[this->influenceIndex];
        }
...
void SkinBrushContext::setColorWithMirror(
...
MStatus SkinBrushContext::doPerformPaint()
{
    ...
    for (const auto &pt : this->mirroredJoinedArray) {
        ...
        float weightMirror = pt.second.second;
        this->setColorWithMirror(
            ptIndex, weightBase, weightMirror, editVertsIndices, multiEditColors, soloEditColors
        );
    }
